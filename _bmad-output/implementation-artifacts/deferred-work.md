...
## Deferred from: launch-readiness pass (Opus 4.8, 2026-06-14) — see launch-readiness-2026-06-14.md
The pass FIXED: favicon 404, `/health` liveness endpoint (FR67), `POST /contact` rate-limit + CAPTCHA fail-closed-in-prod, and the PRD uptime reconciliation (→ 95% / NFR-R1). Empirical Lighthouse: `/`, `/about`, `/contact` pass performance/a11y/SEO ≥0.9 (NFR-P1 <2s met); `/contact` best-practices 0.68 is third-party-embed-bound (lhci best-practices gate relaxed to `warn`). Remaining launch blockers, deferred for owner prioritization:

- **CSP-blocked critical flows (verified, latent regression from removing `'unsafe-inline'`)** — strict CSP silently breaks: (1) **password reset** (`src/views/auth/request-password-reset.ejs:39`, `reset-password.ejs:52` — inline `<script>`, no external fallback → forms never submit; PUBLIC + security-critical, highest priority); (2) Rabbi onboarding **tour** (`dashboard.ejs:286,289` — driver.js from jsdelivr CDN + inline script; core dashboard unaffected); (3) **page/CMS editor** (`admin/pages/edit.ejs:153,330` — Quill from CDN + inline init/style); (4) inert `onclick`/`onsubmit` + inline `style=` across `admin/recordings/list.ejs`, `admin/streaming/index.ejs`, `chat-moderation.ejs`, `recordings/show.ejs`, and the publicly-routed dev `responsive-test.ejs` (`routes/home.js:9`). Fix WITHOUT weakening CSP: move inline JS to `/js/*` (data-attrs for EJS values), vendor driver.js/Quill into `/public`, inline styles → CSS classes.
- **Production env/secrets** — operator must set JWT_SECRET, ENCRYPTION_KEY, TEMPLE_LEGAL_NAME/EIN/ADDRESS, NODE_ENV=production, DATABASE_URL/DB_*, CAPTCHA_*, SMTP_*, APP_URL/APP_BASE_URL, REDIS_URL (full table in the launch-readiness doc). Do NOT set ADMIN_TOKEN.
- **NFR-P6 dashboard <2s** — `adminController.getDashboard` runs ~8 sequential awaits; parallelize with `Promise.all` (not lhci-measured since auth-gated).
- **error.ejs payload mismatch** — controllers passing `{ error }` vs the view's `{ message, title }` render a broken 500 page (no info leak). Standardize.
- **README stale** (Story 1.1) + AGENTS.md route list missing newer mounts; `express.static` lacks `maxAge`.
